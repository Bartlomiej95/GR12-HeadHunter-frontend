...
export const Student = ({filtered, activeFilter, search}: StudentProps) => {
    ...
    const checkUrl = () => {
        if (window.location.href === "http://localhost:3000/") {
            return (
                !activeFilter || search ?
                    students
                        .filter((user: any) => !user.reservationStatus && (user.firstName.toLowerCase().includes(search) || user.lastName.toLowerCase().includes(search) || user.targetWorkCity.toLowerCase().includes(search)))
                    ...
                    // : search ?
                    //     students
                    //         .filter(user => user.firstName === search || user.lastName === search)
                    //         .map((user, index) => (
                    //             <div className="Student">
                    //                 <div className="oneStudent" key={user.id}>
                    //                     <div className="studentName"
                    ...
                    .map((user: Student, index) => (
                        <div className="Student">
                            <div className="oneStudent" key={user.id}>
                                ...
                                <div className="studentButtons">
                                    <NavLink to={`/cv/${user.id}`}><Button text='Pokaż CV' click={() => {
                                    }} id={user.id}/></NavLink>
                                    <Button text='Brak zainteresowania' click={handleRemoveStudentFromTalk}
                                            id={user.id}/>
